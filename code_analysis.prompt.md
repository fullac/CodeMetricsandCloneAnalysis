...
在当前 TypeScript 项目中新增独立的、不依赖 SonarQube 的多语言静态分析 + 代码克隆检测能力。MVP 先覆盖 **Python、Java、C** 三种语言。

## 技术栈

| 层 | 选型 |
|---|---|
| 全栈语言 | TypeScript（复用现有前后端） |
| AST 解析 | `tree-sitter` + `tree-sitter-python` / `tree-sitter-java` / `tree-sitter-c` |
| 静态分析规则 | tree-sitter query（S-expression）→ Findings |
| 静态指标 | AST walk → LOC / 圈复杂度 / 嵌套深度 / 函数数 |
| 克隆检测 | AST 归一化 token → k-gram → MinHash(128维) → LSH(b=16) → Jaccard |
| PDF 报告 | Puppeteer + ReactDOMServer → A4 PDF |
...
排除目录：`node_modules` `dist` `build` `__pycache__` `.venv` `target` `.git` `vendor`。
...
上传 ZIP → 解压 → 收集文件 → 按扩展名路由 parser
  → 每个文件 tree-sitter parse → AST
    ├─ 链路1: query 匹配规则 → Finding[]
    ├─ 链路2: AST walk 计算指标 → FileMetrics[]
    └─ 链路3: token 归一化 → k-gram → MinHash → LSH → ClonePair[]
  → 聚合为 ScanReport → 返回前端 + 可选导出 PDF
```
...
| 方法 | 路径 | 说明 |
|---|---|---|
| POST | `/api/review` | 上传 ZIP + 配置 → 执行分析 → 返回 ScanReport |
...
| 操作 | 文件 | 说明 |
|---|---|---|
| 新建 | `backend/src/services/analysisEngine.ts` | 核心：解析 + 规则 + 指标 + token 归一化 |
| 新建 | `backend/src/services/cloneService.ts` | MinHash + LSH + Jaccard |
| 新建 | `backend/src/services/rules/index.ts` | 规则注册中心 |
| 新建 | `backend/src/services/rules/python.rules.ts` | Python 规则定义 |
| 新建 | `backend/src/services/rules/java.rules.ts` | Java 规则定义 |
| 新建 | `backend/src/services/rules/c.rules.ts` | C 规则定义 |
| 新建 | `backend/src/services/reportService.ts` | PDF 报告生成 |
| 新建 | `frontend/src/components/ScanConfigModal.tsx` | 扫描配置面板 |
| 新建 | `frontend/src/components/FindingsViewer.tsx` | Finding 列表展示 |
| 新建 | `frontend/src/components/CloneDetectionViewer.tsx` | 克隆对展示 |
| 新建 | `frontend/src/components/MetricsDashboard.tsx` | 指标仪表盘 |
| 修改 | `backend/src/types/index.ts` | 新增所有类型定义 |
| 修改 | `backend/src/index.ts` | `/api/review` 改造 + PDF 路由 |
| 修改 | `frontend/src/types.ts` | 同步类型 |
| 修改 | `frontend/src/App.tsx` | 集成配置面板 + 结果展示 |
| 修改 | `backend/package.json` | 添加 tree-sitter 依赖 |
...
本模块**完全独立**于 SonarQube，不依赖 `sonar-scanner`。现有的 SonarQube 扫描功能保留不变，两个分析管线并存：

- SonarQube 管线：`POST /api/review`（sonar 模式，保留原有行为）
- tree-sitter 管线：`POST /api/analyze`（新路由，纯 tree-sitter 分析）

## 约束

- 全部 TypeScript，MinHash/LSH 自行实现
- 一次 tree-sitter parse 复用给三条链路
- 不同语言规则自动匹配，未匹配扩展名跳过
- 规则用独立文件按语言组织，新增语言只需新增规则文件 + parser 依赖
- 支持单个文件、ZIP 包（≤100MB）、GitHub URL 三种上传方式
